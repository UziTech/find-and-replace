...
class FindView {
      ...
      $.div({tabIndex: -1, className: 'find-and-replace'},
        ...
        $.section({className: 'input-block find-container'},
          ...
          $.div({className: 'input-block-item'},
            $.div({className: 'btn-group btn-group-find-prev hidden'},
              $.button({ref: 'prevButton', className: 'btn btn-prev icon icon-arrow-left'})
            ),

            $.div({className: 'btn-group btn-group-find-next'},
        ...
        $.section({className: 'input-block replace-container'},
          ...
          $.div({className: 'input-block-item'},
            $.div({className: 'btn-group btn-group-replace-prev hidden'},
              $.button({ref: 'replacePrevButton', className: 'btn btn-prev icon icon-arrow-left'})
            ),

            $.div({className: 'btn-group btn-group-replace-next'},
  ...
  handleEvents() {
    ...
    this.refs.prevButton.addEventListener('click', () => this.findPrevious({focusEditorAfter: true}));
    this.refs.nextButton.addEventListener('click', (e) => !atom.config.get('temp-find-and-replace.findReplacePrevButton') && e.shiftKey ? this.findPrevious({focusEditorAfter: true}) : this.findNext({focusEditorAfter: true}));
    ...
    this.refs.replacePrevButton.addEventListener('click', () => this.replacePrevious());
    this.refs.replaceNextButton.addEventListener('click', (e) => !atom.config.get('temp-find-and-replace.findReplacePrevButton') && e.shiftKey ? this.replacePrevious() : this.replaceNext());
    ...
    this.subscriptions.add(atom.config.onDidChange('temp-find-and-replace.findReplacePrevButton', () => {
      this.updateButtons()
    }))
  ...
  confirm() {
    this.findNext({focusEditorAfter: atom.config.get('temp-find-and-replace.focusEditorAfterSearch')});
  }

  showPrevious() {
    this.findPrevious({focusEditorAfter: atom.config.get('temp-find-and-replace.focusEditorAfterSearch')});
  }

  liveSearch() {
    let findPattern = this.findEditor.getText();
    if (findPattern.length === 0 || (findPattern.length >= atom.config.get('temp-find-and-replace.liveSearchMinimumCharacters') && !this.model.patternMatchesEmptyString(findPattern))) {
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      if (this.findEditor.getElement().hasFocus() && results > 0 && atom.config.get('temp-find-and-replace.scrollToResultOnLiveSearch')) {
  ...
  updateButtons() {
    const prevButtons = atom.config.get('temp-find-and-replace.findReplacePrevButton');
    this.refs.prevButton.parentElement.classList.toggle('hidden', !prevButtons)
    this.refs.nextButton.classList.toggle('icon', prevButtons)
    this.refs.nextButton.classList.toggle('icon-arrow-right', prevButtons)
    this.refs.nextButton.textContent = (prevButtons ? '' : 'Find')
    this.refs.replacePrevButton.parentElement.classList.toggle('hidden', !prevButtons)
    this.refs.replaceNextButton.classList.toggle('icon', prevButtons)
    this.refs.replaceNextButton.classList.toggle('icon-arrow-right', prevButtons)
    this.refs.replaceNextButton.textContent = (prevButtons ? '' : 'Replace')
  }
  ...
  updateFindEnablement() {
    ...
    if (editor && (!hadFindTooltip || isDisabled)) {
      ...
      this.refs.prevButton.classList.remove('disabled');
      this.refs.nextButton.classList.remove('disabled');

      this.findTooltipSubscriptions.add(atom.tooltips.add(this.refs.prevButton, {
        title: "Find Previous",
        keyBindingCommand: 'find-and-replace:find-prev',
        keyBindingTarget: this.findEditor.element
      }));
      ...
      this.refs.prevButton.classList.add('disabled');
      this.refs.nextButton.classList.add('disabled');

      this.findTooltipSubscriptions.add(atom.tooltips.add(this.refs.prevButton, {
        title: "Find Previous [when in a text document]"
      }));
      this.findTooltipSubscriptions.add(atom.tooltips.add(this.refs.nextButton, {
  ...
  updateReplaceEnablement() {
    ...
    if (canReplace) {
      ...
      this.refs.replacePrevButton.classList.remove('disabled');
  ...
  showWrapIcon(icon) {
    if (!atom.config.get('temp-find-and-replace.showSearchWrapIcon')) return;
    let editor = this.model.getEditor();
    if (!editor) return;
