...
module.exports = class ResultsModel {
  ...
  async search (findPattern, pathsPattern, replacePattern, options = {}) {
    ...
    const leadingContextLineCount = atom.config.get('temp-find-and-replace.searchContextLineCountBefore')
    const trailingContextLineCount = atom.config.get('temp-find-and-replace.searchContextLineCountAfter')

    const startTime = Date.now()
    const useRipgrep = atom.config.get('temp-find-and-replace.useRipgrep')
    const enablePCRE2 = atom.config.get('temp-find-and-replace.enablePCRE2')
