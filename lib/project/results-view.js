...
class ResultsView {
  ...
  async confirmResult({pending} = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await atom.workspace.open(selectedRow.group.result.filePath, {
        pending,
        split: reverseDirections[atom.config.get('temp-find-and-replace.projectSearchResultsPaneSplitDirection')]
      })
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = atom.config.get('temp-find-and-replace.searchContextLineCountBefore');
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = atom.config.get('temp-find-and-replace.searchContextLineCountAfter');
